...
for i = 1: length(Organisms)
    ...
    %models(i).lb(find(model.c)) = 0.05 *growth_single_org(i);
    i = i+1;
end
...
commensalcommunity(end+1) =0;
commensalcommunity(end+1) =0;
X_1 = {'Individual growth','Commensal community', 'Commensal community with pathogens'};
Y_1 = [transpose(growth_single_org),commensalcommunity,gutcommunity]; 
legends = {'B.bifidum';'B.longum';'L.salivarius';'L.reuteri';'F.prasutnzii';'Y.enterolitica';'L.monocytogenes'; 'Community'};
figure(1)
bar(transpose(Y_1))
set(gca,'XTickLabel',X_1)
ylabel('Biomass(gdw)')
legend(legends)
savefig('Biomass growth in a community');
...
ylabel('No.of.Reactions')
legend({'Resistance acquired by Y.enterocolitis', 'Resistance acquired by L.monocytogenes','Resistance acquired by both'})
savefig('Flux_variation');
T.Increasing_flux = Inc_flux; T.Decreasing_flux = Dec_flux;
T.Increasing_exchangeflux = Inc_Exc; T.Decreasing_exchangeflux = Dec_Exc;
disp(T)
...
for j = 1: length(proteinsynthesislevel)
    % Condition 1 : Resistance acquired by Y.enterocolitis
   ...
   % Condition 2 : Resistance acquired by L.monocytogenes
    ...
    % Condition 3 : Resistance acquired by both pathogens
...
% Identifying the reactions which differ in flux reater than 1e-05
...
if length(num) > 1
   ...
   difference_in_flux(:,1) = fluxchangingrxns(:,1);                               % Reactions and its changing flux distribution value
   l =1;
   for l =1 : length(num)
...
% Identifying the reactions which are increasing and decreasing in flux
    ...
    % Common reactions which is being changed across all 3 conditions
